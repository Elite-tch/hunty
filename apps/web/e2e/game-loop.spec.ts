...
test.describe("Core Game Loop: Create → Join → Solve → Complete", () => {
  ...
  test("home page shows active hunts from seed data", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-slot="card-title"]').filter({ hasText: "Draft Hunt" })
    ).toHaveCount(0);
  });
  ...
  test("dashboard shows all hunts including drafts", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-slot="card-title"]').filter({ hasText: "E2E Test Hunt" })
    ).toBeVisible();
    await expect(
      page.locator('[data-slot="card-title"]').filter({ hasText: "Draft Hunt" })
    ).toBeVisible();
  ...
  test("draft hunt shows Activate button only when it has clues", async ({ page }) => {
    await page.goto("/dashboard");
  ...
  test("player can play the test game in preview mode on /hunty", async ({ page }) => {
    await page.goto("/hunty");

    // Fill in a clue with a known answer
    await page.getByPlaceholder("Title of the Hunt").fill("Test Question");
    await page.getByPlaceholder("Description").fill("A simple test question.");
  ...
  test("hunt card on home page has leaderboard link", async ({ page }) => {
    ...
    await expect(huntCard.getByRole("button", { name: /leaderboard/i })).toBeVisible();
  });
  ...
  test("full user journey: register, submit clues, complete hunt", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /Play E2E Test Hunt/i })).toBeVisible({
      timeout: 15000,
    });
  ...
  test("incorrect answer shows feedback without advancing", async ({ page }) => {
    ...
    if (await testBtn.isVisible()) {
      ...
      if (await answerInput.isVisible().catch(() => false)) {
        ...
        await expect(page.locator("text=/incorrect|wrong|try again/i").first()).toBeVisible({
          timeout: 5_000,
        });
  ...
  test("hunt completion persists on leaderboard", async ({ page }) => {
    ...
    const leaderboardBtn = page
      .getByRole("button", { name: /leaderboard|scores|rankings/i })
      .first();
    if (await leaderboardBtn.isVisible().catch(() => false)) {
      await leaderboardBtn.click();
