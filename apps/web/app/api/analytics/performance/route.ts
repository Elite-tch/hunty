...
const PERFORMANCE_STORE_PATH = path.join(process.cwd(), "data", "performance-metrics.json");
...
async function ensureStoreFile(): Promise<void> {
  ...
  try {
    ...
    await fs.writeFile(PERFORMANCE_STORE_PATH, JSON.stringify({ metrics: [] }, null, 2), "utf8");
  }
}
...
async function writeStore(data: StoreData): Promise<void> {
  await ensureStoreFile();
  await fs.writeFile(PERFORMANCE_STORE_PATH, JSON.stringify(data, null, 2), "utf8");
}
