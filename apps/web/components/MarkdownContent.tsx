 ...
 */
export function MarkdownContent({ markdown, fallback = "", className }: MarkdownContentProps) {
  const html = renderMarkdown(markdown?.trim() ? markdown : fallback);

  return (
    <div className={cn("markdown-content", className)} dangerouslySetInnerHTML={{ __html: html }} />
  );
}
