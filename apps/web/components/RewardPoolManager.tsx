...
export function RewardPoolManager({
  huntId,
  isOpen,
  onClose,
}: {
  ...
  const handleSaveDistribution = () => {
    try {
      ...
      setDistributionPlan(
        huntId,
        parsed.map((p) => ({ place: p.place, amount: p.amount }))
      );
      ...
      <DialogContent showCloseButton className="sm:max-w-2xl">
        ...
        <div className="space-y-4">
            ...
            <div className="rounded-lg border p-4">
              ...
              {low && (
                <p className="mt-2 text-sm text-amber-700">
                  Low balance warning: top up recommended
                </p>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <textarea
              aria-label="Distribution Plan (JSON)"
              value={distributionJson}
              onChange={(e) => setDistributionJson(e.target.value)}
              className="w-full h-36 rounded-md border p-2 font-mono text-sm"
            />
          ...
          <div>
            ...
            <div className="mt-2 space-y-1">
              ...
              {(pool?.distribution ?? []).length === 0 && (
                <p className="text-sm text-slate-500">No distribution configured.</p>
              )}
            </div>
          </div>
        </div>
