...
export function RewardHistorySection({
  ...
  const summary = useMemo(() => {
    const totalXlm = entries
      .filter((entry) => entry.type === "XLM")
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        ...
        <div className="grid grid-cols-3 gap-3 sm:grid-cols-3">
          <Card className="rounded-3xl border border-slate-200 bg-slate-50 p-4 shadow-sm">
            ...
            <CardContent className="p-0 mt-3 text-2xl font-semibold text-slate-900">
              {summary.totalTx}
            </CardContent>
          </Card>
          <Card className="rounded-3xl border border-slate-200 bg-slate-50 p-4 shadow-sm">
            <CardTitle className="text-sm text-slate-500">XLM Total</CardTitle>
            <CardContent className="p-0 mt-3 text-2xl font-semibold text-emerald-700">
              {summary.totalXlm.toFixed(2)}
            </CardContent>
          </Card>
          <Card className="rounded-3xl border border-slate-200 bg-slate-50 p-4 shadow-sm">
            <CardTitle className="text-sm text-slate-500">NFT Events</CardTitle>
            <CardContent className="p-0 mt-3 text-2xl font-semibold text-violet-700">
              {summary.totalNfts}
            </CardContent>
          </Card>
        </div>
        ...
        {filteredEntries.length === 0 ? (
          ...
          <div className="divide-y divide-slate-200">
            {filteredEntries.map((entry) => (
              <div
                key={entry.id}
                className="grid grid-cols-[1.7fr_0.7fr_0.7fr_1.1fr_1.4fr] gap-0 px-4 py-4 items-center text-sm text-slate-700"
              >
                ...
                <div className="flex justify-end">
                  <Button asChild variant="outline" className="h-9 rounded-full text-xs px-3">
