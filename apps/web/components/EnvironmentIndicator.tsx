...
export function EnvironmentIndicator() {
  ...
  const bgColor = config.isStaging
    ? "bg-yellow-100 dark:bg-yellow-900"
    : "bg-blue-100 dark:bg-blue-900";
  const textColor = config.isStaging
    ? "text-yellow-800 dark:text-yellow-200"
    : "text-blue-800 dark:text-blue-200";
  const borderColor = config.isStaging
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <div
          className={`w-2 h-2 rounded-full ${config.isStaging ? "bg-yellow-600" : "bg-blue-600"}`}
        />
...
export function EnvironmentBadge() {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-medium ${bgColor} ${textColor} border ${borderColor}`}
    >
