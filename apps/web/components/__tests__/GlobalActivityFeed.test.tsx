...
describe("GlobalActivityFeed", () => {
  ...
  it("shows loading skeletons while data is being fetched", () => {
    ...
    vi.spyOn(activityFeedModule, "getRecentActivity").mockReturnValue(new Promise(() => {}));
  ...
  it("renders activity event items with anonymized addresses and hunt titles", async () => {
    ...
    expect(
      screen.queryByText("GDQP2KPQGKIHYJGXNUIYOMHARUARCA7DJT5FO2FFOOKY3B2WSQHG4W37")
    ).toBeNull();
  ...
  it("displays an empty state message when there are no events", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/be the first to complete a hunt/i)).toBeInTheDocument();
    });
  });
  ...
  it("re-fetches data after the polling interval", async () => {
    vi.useFakeTimers();
    const spy = vi.spyOn(activityFeedModule, "getRecentActivity").mockResolvedValue(MOCK_EVENTS);
  ...
  it("shows a toast when a new HuntCompleted event arrives", async () => {
    ...
    expect(toastSuccess).toHaveBeenCalledWith("GDE...TYUI completed Market Mayhem!", {
      duration: 4000,
    });
  });
  ...
  it("does not crash when fetch fails", async () => {
    vi.spyOn(activityFeedModule, "getRecentActivity").mockRejectedValue(new Error("Network Error"));
