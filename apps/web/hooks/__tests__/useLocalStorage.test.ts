...
describe("useLocalStorage", () => {
  ...
  it("persists the new value to localStorage when the setter is called", () => {
    ...
    expect(localStorageMock.setItem).toHaveBeenCalledWith("count", JSON.stringify(99));
  ...
  it("supports functional updates (same API as useState)", () => {
    ...
    expect(localStorageMock.setItem).toHaveBeenCalledWith("counter", JSON.stringify(15));
  });
  ...
  it("handles array values correctly", () => {
    ...
    expect(localStorageMock.setItem).toHaveBeenCalledWith("tags", JSON.stringify(["a", "b", "c"]));
  });
});
