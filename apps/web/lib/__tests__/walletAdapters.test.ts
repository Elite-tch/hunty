...
import { getAlbedoPublicKey, signWithAlbedo, createAlbedoAdapter } from "../wallets/albedoAdapter";
import { getXBullPublicKey, signWithXBull, createXBullAdapter } from "../wallets/xbullAdapter";
...
describe("freighterAdapter", () => {
  ...
  describe("getFreighterPublicKey", () => {
    ...
    it("throws when freighter returns no address", async () => {
      ...
      await expect(getFreighterPublicKey()).rejects.toThrow("Freighter wallet not available");
    });
  });
...
describe("albedoAdapter", () => {
  ...
  it("throws when albedo returns no pubkey", async () => {
    ...
    vi.stubGlobal("window", { albedo });
    await expect(getAlbedoPublicKey()).rejects.toThrow("Albedo did not return a public key");
  });
  ...
  it("throws when albedo tx() returns no signed XDR", async () => {
    ...
    vi.stubGlobal("window", { albedo });
    await expect(signWithAlbedo("raw_xdr")).rejects.toThrow("Albedo did not return signed XDR");
  });
