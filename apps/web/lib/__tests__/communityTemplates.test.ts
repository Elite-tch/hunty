...
function validInput(overrides: Partial<CommunityTemplateInput> = {}): CommunityTemplateInput {
  return {
...
describe("community templates", () => {
  ...
  it("persists a valid submission and reads it back", () => {
    ...
    expect(getCommunityTemplateBySlug(saved.slug)?.title).toBe("Riverside Photo Quest");
  });
  ...
  it("rejects submissions with a missing author or short title", () => {
    expect(validateCommunityTemplateInput(validInput({ author: "" }))).toMatch(/name/i);
    expect(validateCommunityTemplateInput(validInput({ title: "Hi" }))).toMatch(/title/i);
  });
