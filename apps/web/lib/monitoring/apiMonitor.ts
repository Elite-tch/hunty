...
export function recordMetric(metric: ApiMetric): void {
  ...
  if (metric.durationMs > MONITORING.apis.slowRequestThresholdMs) {
    logger.warn(
      `[Monitoring] Slow request: ${metric.method} ${metric.route} took ${metric.durationMs}ms`
    );
  }

  if (metric.statusCode >= 500 && Math.random() < MONITORING.apis.errorSampleRate) {
    logger.error(
      `[Monitoring] Server error: ${metric.method} ${metric.route} returned ${metric.statusCode}`,
      metric.error
    );
  }
}
...
export function getAggregatedMetrics() {
  const routeStats = new Map<
    string,
    { count: number; errors: number; totalDuration: number; maxDuration: number }
  >();

  for (const m of metrics) {
    const existing = routeStats.get(m.route) ?? {
      count: 0,
      errors: 0,
      totalDuration: 0,
      maxDuration: 0,
