...
export function createHunt(overrides: HuntFactoryOverrides = {}): StoredHunt {
  ...
  const endTime =
    overrides.endTime ?? startTime + faker.number.int({ min: 2 * 86400, max: 10 * 86400 });
  ...
  return {
    ...
    status:
      overrides.status ??
      faker.helpers.arrayElement(["Active", "Draft", "Completed", "Cancelled"] as const),
...
export function createClue(overrides: ClueFactoryOverrides = {}): Clue {
  return {
    ...
    difficulty:
      overrides.difficulty ?? faker.helpers.arrayElement(["Easy", "Medium", "Hard"] as const),
...
export function createPlayer(overrides: PlayerFactoryOverrides = {}): TestPlayer {
  return {
    ...
    joinedAt:
      overrides.joinedAt ??
      Math.floor(Date.now() / 1000) - faker.number.int({ min: 86400, max: 30 * 86400 }),
    ...overrides,
  };
}
...
export function createSharedFixtures() {
  ...
  const activeClues = [
    ...
    createClue({
      ...
      question: "Which landmark is hidden nearby?",
      answer: "statue",
    }),
  ];
  ...
  return {
    ...
    completedScenario: {
      ...
      clues: [
        createClue({
          huntId: completedHunt.id,
          question: "Where was the reward hidden?",
          answer: "garden",
        }),
      ],
