...
describe("uploadNftMetadata", () => {
  ...
  it("throws IpfsUploadError on non-2xx response", async () => {
    ...
    await expect(uploadNftMetadata(VALID_METADATA, WALLET)).rejects.toBeInstanceOf(IpfsUploadError);
  });
...
describe("MetadataValidationError", () => {
  it("includes error summary in message", () => {
    const err = new MetadataValidationError([{ field: "name", message: "name is required" }]);
