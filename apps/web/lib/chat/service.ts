...
export const addChatMessage = (
  ...
  localStorage.setItem(getChatMessagesKey(huntId), JSON.stringify(updatedMessages));
...
export const updateChatSettings = (
  ...
  localStorage.setItem(getChatSettingsKey(huntId), JSON.stringify(updatedSettings));
...
export const unmutePlayer = (huntId: number, address: string): string[] => {
  const currentSettings = getChatSettings(huntId);
  const mutedAddresses = currentSettings.mutedAddresses.filter((addr) => addr !== address);
  updateChatSettings(huntId, { mutedAddresses });
  return mutedAddresses;
};
...
export const reportMessage = (
  ...
  localStorage.setItem(getReportedMessagesKey(), JSON.stringify(updatedReports));
