...
export function detectRankChanges(
  ...
  const playersWhoOvertook = sorted.slice(0, currentRank).filter((e) => {
    const wasBehindEntry = previousSnapshot.entries.find(
      (pe) => pe.address.toLowerCase() === e.address.toLowerCase() && pe.rank > previousEntry.rank
    );
  ...
  if (playersWhoOvertook.length > 0) {
    for (const overtaker of playersWhoOvertook) {
      ...
      const alreadyAdded = notifications.some(
        (n) =>
          n.type === "overtaken" &&
          n.overtakenBy === (overtaker.name || overtaker.address.slice(0, 6))
      );
      if (!alreadyAdded) {
        notifications.push({
