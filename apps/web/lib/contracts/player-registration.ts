 ...
 */
async function withRetry<T>(fn: () => Promise<T>, retryConfig = RETRY_CONFIG): Promise<T> {
  try {
...
function getWallet() {
  if (typeof window === "undefined") {
    throw new RegistrationError("Browser environment required", "BROWSER_REQUIRED");
  }
            ...
            if (body?.data) {
              ...
              try {
                ...
                if (serverProgress.currentClueIndex > progress.currentClueIndex) {
                  advanceHuntProgress(
                    huntId,
                    serverProgress.currentClueIndex,
                    serverProgress.totalClues
                  );
                }
 ...
 */
const registrationStatusCache = new Map<
  string,
  {
