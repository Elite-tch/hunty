...
export function ensureOwner(huntId: number, ownerAddress: string): HuntCollaborator {
...
export type InviteResult =
  { ok: true; collaborator: HuntCollaborator } | { ok: false; error: string };
...
export function updateCollaboratorRole(
  ...
  const updated = list.map((c) => (c.walletAddress === targetAddress ? { ...c, role } : c));
  saveCollaborators(huntId, updated);
  appendActivity(huntId, {
    actorAddress,
    action: "role_changed",
