...
describe("useLocalStorage", () => {
  ...
  it("persists the new value when setValue is called", async () => {
    const adapter = makeAdapter();
    const { result } = renderHook(() => useLocalStorage("count", 0, adapter));
  ...
  it("supports functional updater form", () => {
    const adapter = makeAdapter();
    const { result } = renderHook(() => useLocalStorage("n", 1, adapter));
  ...
  it("works with object values", async () => {
    const adapter = makeAdapter();
    const { result } = renderHook(() => useLocalStorage<{ x: number }>("obj", { x: 0 }, adapter));
  ...
  it("returns initialValue and skips storage when adapter is null", async () => {
    const { result } = renderHook(() => useLocalStorage("key", "default", null));
    await act(async () => {});
  ...
  it("keeps initialValue on corrupt JSON in storage", async () => {
    ...
    const { result } = renderHook(() => useLocalStorage("bad", 99, adapter));
    await act(async () => {});
